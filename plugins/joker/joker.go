...
func Register(session *service.Session) {
	session.HandlerRegister.Add(service.MSG_TEXT, service.Handler(listenCmd), "joker")
	go fetchJokes()
...
func listenCmd(session *service.Session, msg *service.ReceivedMessage) {
	...
	case <-time.After(time.Second * 5):
		return
	case joke := <-jokeQueue:
		session.SendText(joke, session.Bot.UserName, service.RealTargetUserName(session, msg))
	}
}
